...
export class DataService {
  ...
  getBooks(): Observable<any> {
    return this.http.get('/books').map(resp => resp.json()).catch(this.onError)
  }
  ...
  addEditBook(book: Book): Observable<any> {
    return this.http.put(`/books/${book.isbn}`, JSON.stringify(book), this.options)
    .map(this.mapResponseStatus).catch(this.onError)
  }

  deleteBook(book: Book): Observable<any> {
    return this.http.delete(`/books/${book.isbn}`).map(this.mapResponseStatus).catch(this.onError)
  }
